...
function PostCard({ post, fetchType }) {
  ...
  const handleSubmitComment = async () => {
    ...
    if (fetchType === "GET_ALL_POSTS") {
      await dispatch(getWorkoutPosts());
    }
  ...
  const handleSubmit = async () => {
    ...
    await dispatch(updateWorkoutPostById(newPost));
    if (fetchType === "GET_ALL_POSTS") {
      await dispatch(getWorkoutPosts());
    }
    if (fetchType === "GET_ALL_USER_POSTS") {
    ...
    if (fetchType === "GET_ALL_USER_POSTS") {
      await dispatch(getWorkoutPostById(user.userId));
    }
    setEditable(false);
  };
  ...
  const handleLikePost = async () => {
    ...
    await dispatch(likeWorkoutPostById(likedPost));
    if (fetchType === "GET_ALL_POSTS") {
      await dispatch(getWorkoutPosts());
    }
    if (fetchType === "GET_ALL_USER_POSTS") {
      await dispatch(getWorkoutPostById(post.userId));
      await dispatch(getPostShareByUserId(user.userId));
    }
    if (fetchType === "GET_ALL_POSTS_USER") {
      await dispatch(getWorkoutPostById(post.userId));
              ...
              {!editable && (
                <>
                  ...
                  <AiFillDelete
                    ...
                    size={25}
                    onClick={() => {
                      dispatch(deleteWorkoutPostById(post.id));
                    }}
                  />
                </>
              )}
