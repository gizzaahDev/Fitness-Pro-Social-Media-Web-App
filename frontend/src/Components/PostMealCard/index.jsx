...
function PostCard({ post, fetchType }) {
  ...
  const handleSubmit = async () => {
    ...
    await dispatch(updateMealPostById(newPost));
    if (fetchType === "GET_ALL_POSTS") {
      await dispatch(getMealPosts());
    }
    if (fetchType === "GET_ALL_USER_POSTS") {
    ...
    if (fetchType === "GET_ALL_USER_POSTS") {
      await dispatch(getMealPostById(user.userId));
    }
    setEditable(false);
  };
  ...
  const handleLikePost = async () => {
    ...
    await dispatch(likeMealPostById(likedPost));
    if (fetchType === "GET_ALL_POSTS") {
      await dispatch(getMealPosts());
    }
    if (fetchType === "GET_ALL_USER_POSTS") {
      await dispatch(getMealPostById(post.userId));
      await dispatch(getPostShareByUserId(user.userId));
    }
    if (fetchType === "GET_ALL_POSTS_USER") {
      await dispatch(getMealPostById(post.userId));
              ...
              {!editable && (
                <>
                  ...
                  <AiFillDelete
                    ...
                    size={25}
                    onClick={() => {
                      dispatch(deleteMealPostById(post.id));
                    }}
                  />
                </>
              )}
